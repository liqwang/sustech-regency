      ...
      <div v-for="i in hs.hotels.length">
        <el-sub-menu :index="hs.hotels[i - 1].cityName" v-if="ss.values1[i - 1]">
          ...
          <div v-for="i2 in hs.hotels.length">
            <el-menu-item-group :title="hs.hotels[i2 - 1].regionName"
              v-if="rs.values1[i2 - 1] && hs.hotels[i2 - 1].cityName == hs.hotels[i - 1].cityName"
              :index="hs.hotels[i2 - 1].regionName">
              <div v-for="i3 in hs.hotels.length">
                <el-menu-item :index="hs.hotels[i3 - 1].id"
                  v-if="hs.hotels[i3 - 1].regionName == hs.hotels[i2 - 1].regionName">
                  {{ hs.hotels[i3 - 1].name }}
                </el-menu-item>
              </div>
            </el-menu-item-group>
...
const selectMenu = (index: string, path: string[], item: string) => {
